...
fetch_key() {
    ...
    if [ "${ext}" = "der" ]; then
        # Each individual certificate needs to be extracted into its own file
        # as the der format assumes a single certificate per file.
        i=0
        for certdata in $(cat "${DEST_DIR}/${3}"); do
            i=$(expr $i + 1)
            dest_file=$(echo "${3}" | sed -e "s,\.${ext}$,-${i}.${ext},")
            echo "${certdata}" | base64 -d > "${DEST_DIR}/${dest_file}"
        done
        rm -f "${DEST_DIR}/${3}"
    fi
}
